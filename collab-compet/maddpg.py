...
from utils.metrics import MultiAgentMetrics
...
class NaiveMADDPG:
    def __init__(self, env: UnityEnvironment, state_space: int, action_space: int,
        ...
        self._actor = actor_network_builder(state_space, action_space).to(device=self._device,
                                                                          dtype=self._dtype).train()
        ...
        self._critic = critic_network_builder(state_space, action_space).to(device=self._device,
                                                                            dtype=self._dtype).train()
        self._critic_target = critic_network_builder(state_space, action_space).to(device=self._device,
                                                                                   dtype=self._dtype)
        ...
        self._metrics = MultiAgentMetrics()
        ...
        self._noise = [OrnsteinUhlenbeckProcess(dims=self._action_space, mu_start=0., mu_final=0.,
    ...
    def control(self, state: State, explore: bool = False) -> Action:
        ...
        if explore and self._metrics.step < 1000000:
            noise = np.array([n.sample() for n in self._noise])
            action += torch.tensor(noise, device=self._device, dtype=self._dtype)
            for i, action_noise in enumerate(noise.flatten().tolist()):
    ...
    def eval(self, checkpoint: str, num_episodes: int = 100, step_delay: Optional[float] = None):
        ...
        while True:
            ...
            state = torch.tensor(env_info.vector_observations, device=self._device, dtype=self._dtype)

            while True:
                action = self.control(state, explore=False)
                env_info = self._env.step(action.cpu().numpy())[self._brain_name]
                next_state = torch.tensor(env_info.vector_observations, device=self._device, dtype=self._dtype)
    ...
    def train(self):
        while True:
            ...
            while True:
                ...
                if any(done):
                    next_state = (TerminalState, TerminalState)

                self._store_transition(state[0], action[0], next_state[0], reward[0])
                self._store_transition(state[1], action[1], next_state[1], reward[1])
...
if __name__ == '__main__':
    ...
    env = UnityEnvironment(file_name=os.path.join(os.curdir, 'bin', 'Tennis.x86_64'), no_graphics=False)
    tb = init_tensorboard_logger(os.path.join(os.pardir, 'tensorboard'), get_run_id())
    algo = NaiveMADDPG(env=env, state_space=24, action_space=2,
